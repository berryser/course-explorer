...
(function () {
  ...
  class Course {
    constructor(fields) {
      ...
      this.postedTime = fields.postedTime ?? fields.semester ?? null; // alias handling
    ...
    static fromRaw(obj) {
      ...
      if (!id || !title) {
        return null; // critical fields missing; skip
      }
      return new Course({
        id,
        title,
    ...
    static semesterRank(semesterStr) {
      ...
      // Expected "Winter YYYY", "Spring YYYY", "Summer YYYY", "Fall YYYY"
      const match = /^(\w+)\s+(\d{4})$/.exec(trimmed);
      if (!match) return null;
      const season = match[1];
      const year = Number(match[2]);
      const seasonIndex = SEASON_ORDER[season];
